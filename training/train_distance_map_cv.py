#!/usr/bin/env python
"""Converted from Training/3. Distance Map CV.ipynb.
...
CV_WEIGHT_PATH = WEIGHT_PATH / "distance map CV"
...
for fold, split in enumerate(active_fold_splits, 1):
    ...
    # 3) losses & optimiser
    loss_fn_seg  = BoundaryWeightedTverskyLoss()
    loss_fn_reg  = FocalMSELoss(gamma=2.5, reduction='mean')
    combined_loss_func = UncertaintyLoss().to(device)

    optimiser = optim.AdamW(
        list(model.parameters()) + list(combined_loss_func.parameters()),
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # ------------ TRAIN ------------
        model.train(); epoch_loss = 0.0; dice_metric_train.reset()
        for batch_idx, batch in enumerate(train_loader, 1):
            ...
            with autocast(device_type="cuda", enabled=True):
                ...
                if tgt_dist.shape[2:] != dist_pred.shape[2:]:
                    tgt_dist_res = F.interpolate(tgt_dist, size=dist_pred.shape[2:],
                ...
                loss     = combined_loss_func(loss_seg, loss_reg)
        ...
        print(f"   Train ‖ Loss {epoch_loss:.4f}  Dice {train_dice:.4f}")

        # ------------ VALIDATE ------------
        model.eval(); val_loss = 0.0; reg_rmse_val = 0.0; dice_metric_val.reset()
        with torch.no_grad():
            for val_batch_idx, vbatch in enumerate(val_loader, 1):
                ...
                if v_dist.shape[2:] != v_dist_pred.shape[2:]:
                    v_dist_res = F.interpolate(v_dist, size=v_dist_pred.shape[2:],
                                               mode="trilinear", align_corners=False)
                else:
                    v_dist_res = v_dist
                ...
                vloss     = combined_loss_func(vloss_seg, vloss_reg)
                val_loss += vloss.item()
        ...
        print(f"   Val   ‖ Loss {val_loss:.4f}  Dice {val_dice:.4f}  RMSE {reg_rmse_val:.4f}")
